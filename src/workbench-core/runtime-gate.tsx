...
export function WorkbenchRuntimeGate({
  ...
  useEffect(() => {
    ...
    const resolveStartup = async () => {
      let projection = await resolve();
      // The exact Host may still be binding immediately after launch. Retry
      // only when the owner marks this status retryable, then show its result.
      for (const delay of [300, 900, 1800]) {
        if (!active || projection.status === 'ready' || !projection.retryable) break;
    ...
    void resolveStartup().then((projection) => {
      if (!active) return;
  ...
  }, [clear, offlineCoordinator, reloadKey, resolve, toErrorMessage]);
