...
- This fork is being integrated as the standalone Nimi App `openmontage.studio`.
- Use the published app-tools existing-App workflow: install dependencies, then `pnpm run sync` and `pnpm run check`. The initial source was generated by the official scaffold, but this existing repository retains ownership of its README, AGPL license, ignore rules, and product code; it has no managed scaffold lock.
- The Nimi platform account and any OpenMontage business account are separate. Do not collect Nimi credentials or use user-display fields as an application login ticket.
- Nimi operations go through the standard SDK/Kit Local App carrier. Runtime owns App Access and AI execution; the App owns production decisions and approval. Python media helpers receive no Nimi credentials or generic Runtime forwarding surface.
- Use the official Desktop-supervised App launcher. A renderer URL, direct executable, or running process does not prove protected App Access.
- Preserve the existing Python tools and production artifacts where their contracts fit. The Nimi App may implement its own production controller; do not put that workflow into Nimi Runtime or route AI directly to providers as a fallback.
- Keep implementation and acceptance claims tied to actual checks and real journeys. Unrun product paths remain NOT-VERIFIED.
